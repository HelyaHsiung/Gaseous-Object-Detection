...
class MultiTemporalShift:
    '''
    Shift the multi frames anb boxes along the temporal dimension.
    Args:
        shift_range: the max shift distance. the shift (dx, dy) will
        be in range [-shift_range,shift_range].
    '''
    def __init__(self, mean, to_rgb=True, shift_range=3.0):
    ...
    def __call__(self, results):
        ...
        mean_extend = self.mean * (c//3)
        # speedup expand when meets large image
        if np.all(self.mean == self.mean[0]):
            new_img = np.empty((h, w, c),img.dtype)
            new_img.fill(self.mean[0])
        else:
            new_img = np.full((h, w, c), mean_extend, dtype=img.dtype)

        for i in range(num_frames):
            shift_range_dx = random.randint(-self.shift_range,
                                            self.shift_range)
            shift_range_dy = random.randint(-self.shift_range,
                                            self.shift_range)
            if shift_range_dx>0:
                left = 0
                right = w - shift_range_dx
            else:
                left = 0 - shift_range_dx
                right = w

            if shift_range_dy>0:
                top = 0
                bottom = h - shift_range_dy
            else:
                top = 0 - shift_range_dy
                bottom = h

            # adjust the img
            patch = np.array( (int(left), int(top), int(right), int(bottom)))
            patch_new = np.array( (int(left+shift_range_dx), int(top+shift_range_dy),
                                   int(right+shift_range_dx), int(bottom+shift_range_dy)))
            new_img[patch_new[1]:patch_new[3], patch_new[0]:patch_new[2], i*3+0:i*3+3] = img[patch[1]:patch[3], patch[0]:patch[2], i*3+0:i*3+3]
            boxes = results['gt_bboxes_'+str(i+1)].copy()
            boxes[:, 2:] = boxes[:, 2:].clip(max=patch[2:])
            boxes[:, :2] = boxes[:, :2].clip(min=patch[:2])
            boxes[:,0] += shift_range_dx
            boxes[:,1] += shift_range_dy
            boxes[:,2] += shift_range_dx
            boxes[:,3] += shift_range_dy

            results['gt_bboxes_'+str(i+1)] = boxes
            results['img'] = new_img
        return results
...
@PIPELINES.register_module()
class MultiFramesNormalize:
    """Normalize the multi images in GODVideo dataset.
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            for i in range(img.shape[2]//3):
                #img_pro = img[:,:,i*3:i*3+3]
                #img_pro =  mmcv.imnormalize(img_pro, self.mean, self.std, self.to_rgb)
                #img[:,:,i*3:i*3+3] =  img_pro #mmcv.imnormalize(img[:,:,i*3:i*3+3], self.mean, self.std, self.to_rgb)
                img[:,:,i*3:i*3+3] =  mmcv.imnormalize(img[:,:,i*3:i*3+3], self.mean, self.std, self.to_rgb)
            results[key] = img
...
@PIPELINES.register_module()
class MultiPhotoMetricDistortion:
    """Apply photometric distortion to  multi images for GODVideo dataset sequentially, every transformation
    ...
    # 8. randomly swap channels  -> delete
    ...
    def __call__(self, results):
        ...
        for i in range(img.shape[2]//3):
            img[:,:,i*3:i*3+3] = mmcv.bgr2hsv(img[:,:,i*3:i*3+3])

        # random saturation
        if random.randint(2):
            # img[..., 1] *= random.uniform(self.saturation_lower,
            #                               self.saturation_upper)
            randomsaturation_value = random.uniform(self.saturation_lower, self.saturation_upper)
            for i in range(img.shape[2]//3):
                img[..., i*3+1] *= randomsaturation_value
        ...
        if random.randint(2):
            ...
            # img[..., 0][img[..., 0] > 360] -= 360
            # img[..., 0][img[..., 0] < 0] += 360
            randomhue_value = random.uniform(-self.hue_delta, self.hue_delta)
            for i in range(img.shape[2]//3):
                img[..., i*3+0] += randomhue_value
                img[..., i*3+0][img[..., i*3+0] > 360] -= 360
                img[..., i*3+0][img[..., i*3+0] < 0] += 360

        # convert color from HSV to BGR
        # img = mmcv.hsv2bgr(img)
        for i in range(img.shape[2]//3):
            img[:,:,i*3:i*3+3] = mmcv.hsv2bgr(img[:,:,i*3:i*3+3])
...
@PIPELINES.register_module()
class MultiExpand:
    """Random expand the multi images & bboxes.
        ...
        while True:
            ...
            for i in range(50):
                ...
                if len(overlaps) > 0:
                    ...
                    # if not mask.any():#ori: one box of this frame satisfy in patch is ok;
                    #     continue
                    if not mask.all():#now : all box of multi frames must in patch ;
                        continue
                    for key in results.get('bbox_fields', []):
