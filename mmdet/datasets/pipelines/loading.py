    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            filename = osp.join(results['img_prefix'],
                                results['ann_list'][0]['img_info_'+str(len(results['ann_list']))]['filename'])
        else:
            filename = results['ann_list'][0]['img_info_'+str(len(results['ann_list']))]['filename']

        frame_order = int(filename.split('/')[-1].split('_')[1].split('.')[0].lstrip('0'))
        imglist = None

        for t in range(self.n_segment,0,-1):
            #filename:'data/IOD-Video_COCO/train/062_00220.png'
            filenewname = filename[:-9] + str(frame_order - t + 1).zfill(5) + '.' + filename.split('.')[1]
            # if frame_order - t + 1==0:
            #     print(filename)
            #     print(frame_order)
            #     print(results)

            img_bytes = self.file_client.get(filenewname)
            ...
            if imglist is None:
                ...
                imglist = np.concatenate((imglist, img), axis=2)


        results['filename'] = filename
        results['ori_filename'] =  results['ann_list'][0]['img_info_'+str(len(results['ann_list']))]['filename']#results['img_info']['filename']
        results['img'] = imglist #img
...
class LoadMultiAnnotations:
    ...
    def _load_bboxes(self, results):
        ...
        """
        for i in range(len( results['ann_list'])):
            n_segment = len( results['ann_list'])

            ann_info = results['ann_list'][i]['ann_info_'+str(n_segment-i)]#results['ann_info']
            results['gt_bboxes_'+str(n_segment-i)] = ann_info['bboxes'].copy()

            if self.denorm_bbox:
                bbox_num = results['gt_bboxes_'+str(n_segment-i)].shape[0]
                if bbox_num != 0:
                    h, w = results['img_shape'][:2]
                    results['gt_bboxes_'+str(n_segment-i)][:, 0::2] *= w
                    results['gt_bboxes_'+str(n_segment-i)][:, 1::2] *= h

            gt_bboxes_ignore = ann_info.get('bboxes_ignore', None)
            if gt_bboxes_ignore is not None:
                results['gt_bboxes_ignore'] = gt_bboxes_ignore.copy()
                results['bbox_fields'].append('gt_bboxes_ignore')
            results['bbox_fields'].append('gt_bboxes_'+str(n_segment-i))
    ...
    def _load_labels(self, results):
        ...
        results['gt_labels'] = results['ann_list'][0]['ann_info_'+str(len(results['ann_list']))]['labels'].copy()
        return results
