...
def load_imagenet_pretrained_model(opt, model):
    ...
    if arch == 'dla_34':
    ...
    elif arch == 'TINresnet_50' or arch == 'TSMresnet_50' or arch == 'TIN2resnet_50':
        ...
        for key, value in state_dict_copy.items():
            ...
            state_dict['backbone.' + key] = value
            if 'conv2' in key:
                 # state_dict['backbone.' + key] = value.unsqueeze(2).repeat(1,1,3,1,1)#1221
                 #print(value.size())
                 inputC,outputC,kernelh,kernelw = value.shape
                 # key_deformconv = key.replace('conv2','Deform_Conv3d')
                 # state_dict['backbone.' + key_deformconv] = value.unsqueeze(2).repeat(1,1,3,1,1)#[:,0:outputC//32,:,:]
                 # key_deformconv = key.replace('conv2','Deformed2DFlow.conv2_2')
                 # state_dict['backbone.' + key_deformconv] = value#[:,0:outputC//32,:,:]
                 # key_deformconv = key.replace('conv2','Deformed2DFlow.conv2_3')
                 # state_dict['backbone.' + key_deformconv] = value#[:,0:outputC//32,:,:]
                 #key_deformconv = key.replace('conv2','Deformed2DFlow.Deform_Conv3d')#'deform_conv2d_naive.deform_conv2d_naive')
                 #value_3d = value.unsqueeze(2).repeat(1,1,3,1,1) #64, 64, 3, 3 -> 64, 64, 3, 3 ,3
                 #state_dict['backbone.' + key_deformconv] = value_3d#backbone.layer3.3.Deformed2DFlow.Deform_Conv3d.weight
            #if 'conv2.bias' in key:
            #     print(value.size())
            #     key_deformconv = key.replace('conv2.bias','Deformed2DFlow.Deform_Conv3d.bias')#'deform_conv2d_naive.deform_conv2d_naive')      
            #     value_3d_bias = value.unsqueeze(2).repeat(1,1,3,1,1) #64, 64, 3, 3 -> 64, 64, 3, 3 ,3
            #     state_dict['backbone.' + key_deformconv] = value_3d_bias
                 
                 #Deformed2DFlow.Deform_Conv3d.bias 
                 #print(new_model_state_dict[key_deformconv].size())
        #for i in new_model_state_dict.keys():
        #    print(i)
        # model.load_state_dict(state_dict, False)#strict=False)
