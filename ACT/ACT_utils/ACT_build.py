...
def load_frame_detections(opt, dataset, K, vlist, inference_dir):
    ...
    for iv, v in enumerate(vlist):
        ...
        for i in range(K, 1 + dataset._nframes[v] ):
            v_num =  v.split('/')[1].split('_')[0]
            pkl = os.path.join(inference_dir, v_num, "{:0>5}.pkl".format(i))#v
            ...
            for label in dets:
                ...
                for k in range(K):
                    if (i - K + 1 + k >= 0):
                        vdets[i - K + 1 + k] = np.concatenate((vdets[i - K + 1 + k], np.concatenate((tubelets[:, np.array([4 * k, 1 + 4 * k, 2 + 4 * k, 3 + 4 * k, -1])], labels), axis=1)), axis=0)
