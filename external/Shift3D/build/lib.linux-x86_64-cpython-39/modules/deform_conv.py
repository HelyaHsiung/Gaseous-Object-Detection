...
class DeformConvPack(DeformConv):
    ...
    # def forward(self, input,offset_pre):
    #     offset = self.conv_offset(input) + offset_pre
    def forward(self, input):#,offset_flow):
        offset = self.conv_offset(input) #+ offset_flow
        return DeformConvFunction.apply(input, offset,
                                          self.weight, 
                                          self.bias, 
                                          self.stride, 
...
#original && temp1-> offset && restrict offset dimension dimension='THW'
...
#original &&  && restrict offset dimension dimension='THW'
class DeformConvPack_d(DeformConv_d):
    ...
    def forward(self, input):
        temp = self.conv_offset(input)#B,self.length*3*3*3,t,h,w
        dimension_T = 'T' in self.dimension
        dimension_H = 'H' in self.dimension
        dimension_W = 'W' in self.dimension
        b, c, t, h, w = temp.shape #B,self.length*3*3*3,t,h,w self.length=c=2*27=54
        if self.length == 2:
        ...
        if self.length == 1:
            temp1 = temp.clone()#B,self.length*3*3*3,t,h,w self.length=1 need to extend to self.length=3 *3*3*3
            offset = torch.cat((temp.clone(), temp1, temp1), dim=1)
            if dimension_T == True:
                for i in range(
                        self.deformable_groups * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2]):
